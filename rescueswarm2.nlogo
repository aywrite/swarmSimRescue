...
to setup
  ...
  set base patch 0 min-pycor ;move the UAVS to the base of the map / building entrance
  ask turtles [
    setxy 0 min-pycor 
    set color orange
    set size uav-Size
  ...
  ask base [ask neighbors [set pcolor black]]
  ask base [set pcolor 255]
end
...
to go
  ...
  ask searchers [    
    if (distance base < comRange) and (color = foundColor) and (foundNO != nobody) [ ;check if a message bearing UAV is in range of Base 
      ask foundNO [
        set pcolor brown ;indicates that the rescuers now know the location of the victim
  ...
  ask searchers [ ;main program loop
    ...
    ifelse clearPath? [right random 60] ;if blocked turn a random amount between 0 and 60 degrees
    ...
    if pcolor = blue and (count searchersDone-here) < 1 and (random 100 < detectionChance) [
      hatch-searchersDone 1 [set color rescueColor]
    die
    ]
...
to-report seeVictim
  ...
  if target-patch2 != nobody and [pcolor] of target-patch2 = victimColor [
    set whereVictim target-patch2
  ]
  let target-patch3 patch-right-and-ahead (visionCone / 2) detectionRange
  if target-patch3 != nobody and [pcolor] of target-patch3 = victimColor [
    set whereVictim target-patch3
  ]
  report whereVictim
end
...
to build-rubble
  ...
  ask patches [
    ...
    if random 300 = 0 [set pcolor victimColor]
  ]
  
  ask patches [if (count neighbors with [pcolor = buildingColor] < 1) and random (3) = 0 [set pcolor black]]
...
to uavBehave
  ...
  [
    ...
    if seeVictim != nobody [face seeVictim]
...
to setupVariables
  ...
  set patchsize 1
  ;parameters
  set UAV-size (1.5 / patchSize) 
  set sightRange (1 / patchSize)
  set comRange (10 / patchSize)
  set moveDistance (1 / patchSize)
